(function($) {
	...
	$.fn.select_autocomplete = function(options) {
		...
		return this.each(function() {
			...
			var $this = $(this),
				data = [],
				$input = $('<input type="text" />').css({width: options.width});
			...
			$input.result(function(event, selected_item, formatted) { 
				...
				$($this.find('option[value=' + selected_item.value + ']')[0]).attr('selected', true).select();
                                //$this.find('option[value=' + selected_item.value + ']').select();
			});
